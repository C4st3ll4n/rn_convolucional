...
classificador.fit(prev_treino, classe_treino, batch_size=10,
                  epochs=5, validation_data=(prev_teste, classe_teste))

result = classificador.evaluate(prev_teste, classe_teste)

c = x_teste[randint(0, len(x_teste))].reshape(1,28,28,1)
c = c.astype('float32')
c /= 255
predicao_c = classificador.predict(c)

resultado = np.argmax(predicao_c)

print("O número predito foi o: {}".format(resultado))
